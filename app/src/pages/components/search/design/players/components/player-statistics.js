                ...
                <View key={club} style={{backgroundColor: 'orange'}}>
                    ...
                    <View style={{flexDirection: 'column'}}>
                        ...
                        {Object.keys(Competition[club]).map((year, innerIndex) => (
                            <View key={innerIndex} style={{flexDirection: 'row', backgroundColor: innerIndex % 2 == 0 ? 'red' : 'blue'}}>
                                {/* Check if it's a cup competition and if clubIcons contains the club name */}
                                {Competition === PlayerHistoryPlaceholder.cup && clubIcons[club] && (
                                  <Image source={clubIcons[club].icon} style={{ height: 30, width: 30 }} />
                                )}
        ...
        <View style={{borderWidth: 1, borderColor: 'blue'}}>
            {DisplayCups ? ( 
                <View style={{flex: 1, flexDirection: 'row'}}>
                    <Pressable onPress={() => competitionToggle(false)} style={{flex: 1, backgroundColor: activeCompetition ?  'red' : 'orange'}}>
                        <Text style={{textAlign: 'center', fontSize: 18}}>League History</Text>
                    </Pressable>

                    <Pressable onPress={() => competitionToggle(true)} style={{flex: 1, backgroundColor: activeCompetition ?  'orange' : 'red'}}>
                        <Text style={{textAlign: 'center', fontSize: 18}}>Cup History</Text>
                    </Pressable>
                </View>
            ) :
                <View style={{flex: 1}}>
                    <Text style={{textAlign: 'center', fontSize: 18}}>Player History</Text>
                </View>
            }
        </View>
    )
...
export default function PlayerTeamStatistics({PlayerHistoryPlaceholder}) {
    ...
    const PlayerCupExperience = PlayerHistoryPlaceholder["Cup Experience"] == 0 ? false : true;

    return (
        <ScrollView style={{flex: 5}}>
            <LeagueOrCup DisplayCups={PlayerCupExperience} setCompetition={setCompetition} />

            {competition ? ( <PlayerHistory Competition={PlayerHistoryPlaceholder.cup} /> )
