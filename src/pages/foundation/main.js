...
export default function Main({ LiveTab, LiveNav, SearchInput, setSearchInput, setSearchStatus, searchStatus }) {
  ...
  const RefreshingTitles = [
    ...
    ["Updating News"],
    // Matches
    ["Fetching Favourite Matches", "Fetching Live Matches", "Fetching Upcoming Matches", "Fetching Results"],
    // Favourites
    ["Fetching Matches", "Fetching Players", "Fetching News", "Fetching Tournaments"],
    // Tournaments
    ["Fetching National Tournments", "Fetching International Tournaments", "Fetching International Tournaments"],
    // Statistics 
  ...
  return (
    <View style={[MainStructure.colours, MainStructure.size]}>
      ...
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Text style={[{ marginTop: 10 }, {color: "white"}, {fontSize: 20}]}>Loading...</Text>
      </View>
    ) : (
...
function Content({ Page, Tab, Matches, UserInput, setSearchInput, setSearchStatus }) {  
    ...
    const contentMap = {
      // News section
      news: {
        major: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Major News</Text>,
        regular: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>News</Text>,
        favourite: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Favourite News</Text>
      },
      // Match section
      matches: {
        favourite: (
        ...
        live: (
          <>
            {Matches.length > 0 ? (
              ...
              <Text style={MatchFoundation.noMatch}>Currently No Live Matches</Text>
            )}
          </>
        ),
        upcoming: (
      ...
      // Favourite section
      favourites: {
        matches: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Favourite Matches</Text>,
        players: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Favourite Players</Text>,
        news: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Favourite News</Text>,
        tournaments: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Favourite Tournaments</Text>
      },
      // tournament section
      tournaments: {
        national: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>National Tournaments</Text>,
        intercontinental: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>Intercontinental Tournaments</Text>,
        international: <Text style={[{ color: 'white' }, { fontSize: 20 }]}>International Tournaments</Text>
      },
      // Statistic Section
      statistics: {
        default: (
